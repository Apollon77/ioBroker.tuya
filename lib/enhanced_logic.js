    ...
    return [
        ...
        },
        {
            ...
            onDpSet: (dpValue, isGroupValue) => {
                if (typeof dpValue !== 'string') return '';
                const value = Buffer.from(dpValue, 'base64');
                return value.readUInt16BE(6) * 1000;
            },
        }
    ];
}
        ...
        states: [
            {
                ...
                onDpSet: (dpValue, isGroupValue) => {
                    ...
                    for (let i = 0; i < value.length; i += 4) {
                        ...
                        res.push({
                            alarmCode: alarmTypes[alarmCode] || `unknown (${value.readUInt8(i)})`,
...
function getBitmapLogic(dpId, def) {
    if (!def.property || def.property.type !== 'bitmap' || !def.property.label || !Array.isArray(def.property.label)) return []
    const res = [];
    for (let i = 0; i < def.property.label.length; i++) {
        res.push({
            ...
            onDpSet: (dpValue, isGroupValue) => {
                if (typeof dpValue !== 'number') return '';
                return !!(dpValue & (1 << i));
            },
